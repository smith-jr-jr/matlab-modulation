...
function plotFeatureComparison(mainFig, featureIndex)
    ...
    for i = 1:numMods
        ...
        data.receivedSignalName = '复基带接收信号';
        data.cumulantAnalysis = computeHighOrderCumulants(data.featureSignal);
        data.featureValues = computeFeatureValues(...
            data.cumulantAnalysis.cumulants, data.featureSignal, data.sampleRate);
    ...
    title(handles.featureComparisonAxes, ...
        sprintf('%s 特征值对比（无噪声）', feFieldNames{featureIndex}), ...
