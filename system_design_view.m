function system_design_view
    ...
    uicontrol( ...
        ...
        'BackgroundColor', colors.panel, ...
        'ForegroundColor', colors.text, ...
        'Position', [20 374 120 22]);

    feButtonNames = {'Fe1', 'Fe2', 'Fe3', 'Fe4', 'Fe5', 'Fa', 'Ff'};
    feRow1Width = 64;
    feRow2Width = 88;
    feButtonHeight = 26;
    feButtonGap = 5;
    ...
    for feIdx = 1:7
        ...
        uicontrol( ...
            'Parent', controlPanel, ...
            'Style', 'pushbutton', ...
            'String', feButtonNames{feIdx}, ...
            'FontName', fontName, ...
            'FontSize', 10, ...
            'FontWeight', 'bold', ...
            'Position', [feStartX + col * (btnW + feButtonGap) btnY btnW feButtonHeight], ...
            'Callback', @(~, ~)plotFeatureComparison(mainFig, feIdx));
    end
    ...
    featureComparisonAxes = axes( ...
        'Parent', displayPanel, ...
        'Units', 'normalized', ...
        'Position', [0.07 0.10 0.40 0.35], ...
        'Visible', 'off');
    ...
    state.handles = struct( ...
        ...
        'featureComparisonAxes', featureComparisonAxes, ...
...
function refreshDisplay(mainFig)
    ...
    if isempty(data)
        ...
        set(handles.cumulantText, 'Visible', 'on');
        return;
    end
...
%            轨迹数据及绘图所需元数据。
    ...
    symbolIndices = generateUniformSymbolSequence(modDef.M, params.numSymbols);
    bits = symbolIndicesToBits(symbolIndices, bitsPerSymbol);
...
%   data               - 已完成能量归一化的调制结果结构体。
    ...
    data.averageSymbolEnergy = targetSymbolEnergy;
...
function plotFeatureComparison(mainFig, featureIndex)
    state = guidata(mainFig);
    handles = state.handles;
    colors = state.colors;
    ...
    featureValues = zeros(1, numMods);
    modNames = cell(1, numMods);

    feFieldNames = {'Fe1', 'Fe2', 'Fe3', 'Fe4', 'Fe5', 'Fa', 'Ff'};

    snrDb = state.noiseSnrDb;

    for i = 1:numMods
        data = buildModulatedSignal(modulations(i));
        data.noisyFeatureSignal = addGaussianNoise(data.featureSignal, snrDb);
        data.hasNoise = true;
        data.noiseSnrDb = snrDb;
        data.receivedSignalName = sprintf('复基带接收信号（加噪, SNR = %.0f dB）', snrDb);
        data = updateCumulantAnalysis(data);

        modNames{i} = modulations(i).name;
        featureValues(i) = data.featureValues.(feFieldNames{featureIndex});
    end

    set(handles.featureComparisonAxes, 'Visible', 'on');

    cla(handles.featureComparisonAxes);
    plot(handles.featureComparisonAxes, 1:numMods, featureValues, 'o', ...
        'LineWidth', 1.0, 'MarkerSize', 6, ...
        'Color', colors.primary, ...
        'MarkerFaceColor', colors.primary);
    set(handles.featureComparisonAxes, 'XTick', 1:numMods, 'XTickLabel', modNames);
    set(handles.featureComparisonAxes, 'XTickLabelRotation', 45);
    set(handles.featureComparisonAxes, 'FontSize', 9);
    grid(handles.featureComparisonAxes, 'on');
    title(handles.featureComparisonAxes, ...
        sprintf('%s 特征值对比（SNR = %.0f dB）', feFieldNames{featureIndex}, snrDb), ...
        'FontWeight', 'bold', 'FontSize', 10);
    ylabel(handles.featureComparisonAxes, feFieldNames{featureIndex});

    updateStatus(mainFig, sprintf('状态：%s 特征值对比图已生成', feFieldNames{featureIndex}));
end
